            ...
            case "IEND": { // Image ending chunk (After ALL chunks are parsed, parse the image data)
                ...
                imageData = zlib.inflateSync(imageData, {
                    chunkSize: interlace ?
                        ...
                        Math.max(((byteWidth * depth + 7 >> 3) + 1) * height, zlib.constants.Z_MIN_CHUNK)
                });
                ...
                const byteDepth = 8 / depth;
                const bitmap = depth <= 8 ?
                    imageData.toJSON().data.flatMap(x => util.fill(byteDepth, y => (x >> depth * y) % 2 ** depth).reverse()) :
                    imageData.toJSON().data.map((x, j) => j % 2 == 0 ? ((x << 8) | imageData[j + 1]) : NaN).filter(x => !Number.isNaN(x));

                if (interlace) {
                    const coords = adam7.coords(width, height);
                    json.data = util.groupArray(util.fill(width * height, x => {
                        const index = coords.findIndex(y => y[0] == x % width && y[1] == Math.floor(x / width)) * channels;
                        return index >= 0 ? bitmap.slice(index, index + channels) : util.fill(channels, () => NaN);
                    }).flat(), byteWidth);
                } else json.data = util.groupArray(bitmap, Math.ceil(byteWidth / byteDepth) * byteDepth).map(x => x.slice(0, byteWidth));
                break;
            }
        }
